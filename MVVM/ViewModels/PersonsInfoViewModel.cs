...
public partial class PersonsInfoViewModel : BaseViewModel
{
    ...
    public ObservableCollection<HealthIcon> Icons { get; set; } = [];

    public HealthRecord PersonInfo { get; }
    ...
    private void SetHealthIcons(HealthRecord record)
    {
        try
        {
            ...
            if (record.HasHIV)
                Icons.Add(new HealthIcon { IconSource = "virus.png", Description = "Imunodeficiência" });
        }
        catch (Exception ex)
        {
